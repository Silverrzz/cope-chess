...
interface Run { id: number; stage: 'uniqueness' | 'difficulty' | 'miss_finetuning'; rating_list_id: number | null; settings: Record<string, unknown>; created_at: string; job: ToolJob }
...
interface SuiteDetail extends SuiteSummary { difficulty_run_id: number | null; miss_count: number; puzzles: Puzzle[]; runs: Run[]; engine_results: EngineResult[] }
...
const missFinetuning = reactive({ rating_list_id: 0, engine_ids: [] as number[], seconds: 300, threads: 1, hash_mb: 256 })
...
const latestMissFinetuning = computed(() => suite.value?.runs.find((run) => run.stage === 'miss_finetuning' && Number(run.settings.source_difficulty_run_id) === suite.value?.difficulty_run_id) ?? null)
...
const selectedMissRatingList = computed(() => context.value?.rating_lists.find((item) => item.id === missFinetuning.rating_list_id) ?? null)
const missRatingMap = computed(() => new Map(selectedMissRatingList.value?.ratings.map((rating) => [rating.engine_id, rating.elo]) ?? []))
const missRatedEngines = computed(() => (context.value?.engines ?? []).filter((engine) => missRatingMap.value.has(engine.id)))
...
watch(selectedMissRatingList, (value) => {
  if (!value) {
    missFinetuning.engine_ids = []
    return
  }
  const available = new Set(value.ratings.map((rating) => rating.engine_id))
  missFinetuning.engine_ids = missFinetuning.engine_ids.filter((id) => available.has(id))
})
...
async function loadContext(): Promise<void> {
  try {
    ...
    if (!missFinetuning.rating_list_id) missFinetuning.rating_list_id = context.value.rating_lists[0]?.id ?? 0
  } catch (cause) {
...
async function startMissFinetuning(): Promise<void> {
  if (!suite.value || pending.value) return
  pending.value = 'miss-finetuning'
  error.value = ''
  try {
    const response = await api.post<{ message: string }>(`/api/admin/tools/puzzle-suite-manager/suites/${suite.value.id}/miss-finetuning`, {
      body: {
        engine_ids: missFinetuning.engine_ids,
        rating_list_id: missFinetuning.rating_list_id,
        movetime_ms: Math.round(missFinetuning.seconds * 1000),
        threads: missFinetuning.threads,
        hash_mb: missFinetuning.hash_mb,
      },
    })
    toast.success(response.message)
    await Promise.all([loadSuite(true), loadContext()])
...
async function cancelActiveJob(): Promise<void> {
  const job = activeJob.value
  if (!job || pending.value) return
  const accepted = await confirm({
    title: 'Cancel tool job?',
    message: 'Stop the active searches and release the worker? Results already received will remain in the run history.',
    confirmLabel: 'Cancel job',
    tone: 'danger',
  })
  if (!accepted) return
  pending.value = 'cancel-job'
  error.value = ''
  try {
    const response = await api.post<{ message: string }>(`/api/admin/tools/jobs/${job.id}/cancel`)
    toast.success(response.message)
    await Promise.all([loadSuite(true), loadContext()])
  } catch (cause) {
    error.value = errorText(cause)
...
function toggleMissEngine(engineId: number): void {
  const index = missFinetuning.engine_ids.indexOf(engineId)
  if (index >= 0) missFinetuning.engine_ids.splice(index, 1)
  else missFinetuning.engine_ids.push(engineId)
}
...
function stageLabel(stage: string): string {
  if (stage === 'uniqueness') return 'Unique-solution filter'
  if (stage === 'miss_finetuning') return 'Miss finetuning'
  return 'Difficulty rating'
}

function threadAdjustedElo(engineId: number): number {
  return (missRatingMap.value.get(engineId) ?? 0) + 80 * Math.log2(missFinetuning.threads)
}
      ...
      <main v-else class="workspace">
        ...
        <template v-else-if="suite">
          ...
          <section v-if="activeJob" class="panel telemetry">
            <div class="telemetry-head"><div><span class="live-dot" /><span class="eyebrow">Live worker telemetry</span><h2>{{ stageLabel(activeRun?.stage ?? '') }}</h2></div><span class="telemetry-actions"><span class="status-pill" :class="`status-pill--${activeJob.status}`">{{ statusText(activeJob.status) }}</span><BaseButton size="small" variant="danger" :loading="pending === 'cancel-job'" @click="cancelActiveJob"><template #icon><AppIcon name="x-circle" :size="14" /></template>Cancel job</BaseButton></span></div>
          ...
          <div class="stage-grid">
            ...
            <section class="panel stage-card miss-stage" :class="{ locked: !suite.miss_count, complete: latestMissFinetuning?.job.status === 'completed' }">
              <div class="stage-head"><span class="stage-number">3</span><div><span class="eyebrow">Finetune</span><h2>Miss finetuning</h2><p>Search only puzzles that every Stage 2 engine missed. Effective engine Elo gains 80 points per thread doubling.</p></div><AppIcon v-if="latestMissFinetuning?.job.status === 'completed'" name="check-circle" :size="20" /></div>
              <div class="stage-form">
                <label class="wide">Rating list<select v-model.number="missFinetuning.rating_list_id" :disabled="!!activeRun"><option v-for="item in context?.rating_lists" :key="item.id" :value="item.id">{{ item.name }}</option></select></label>
                <label>Time<input v-model.number="missFinetuning.seconds" type="number" min="0.1" max="3600" step="0.1" /><span>seconds</span></label>
                <label>Threads<input v-model.number="missFinetuning.threads" type="number" min="1" max="1024" /></label>
                <label>Hash<input v-model.number="missFinetuning.hash_mb" type="number" min="1" /><span>MB</span></label>
              </div>
              <div class="engine-picker">
                <div class="picker-head"><strong>High-thread engines</strong><span>{{ missFinetuning.engine_ids.length }} selected · {{ missRatedEngines.length }} rated</span></div>
                <div v-if="missRatedEngines.length" class="picker-list"><label v-for="engine in missRatedEngines" :key="engine.id" :class="{ selected: missFinetuning.engine_ids.includes(engine.id) }"><input type="checkbox" :checked="missFinetuning.engine_ids.includes(engine.id)" :disabled="!!activeRun" @change="toggleMissEngine(engine.id)" /><span><strong>{{ engine.name }}</strong><small>{{ engine.version }} · base {{ Math.round(missRatingMap.get(engine.id) ?? 0) }}</small></span><b>{{ Math.round(threadAdjustedElo(engine.id)) }}</b></label></div>
                <div v-else class="picker-empty">This rating list has no rated engines.</div>
              </div>
              <div class="stage-footer"><span><i :class="{ ready: eligibleWorkers(missFinetuning.threads, missFinetuning.hash_mb) }" />{{ suite.miss_count }} misses · +{{ Math.round(80 * Math.log2(missFinetuning.threads)) }} Elo · {{ eligibleWorkers(missFinetuning.threads, missFinetuning.hash_mb) }} eligible worker{{ eligibleWorkers(missFinetuning.threads, missFinetuning.hash_mb) === 1 ? '' : 's' }}</span><BaseButton variant="primary" :loading="pending === 'miss-finetuning'" :disabled="!!activeRun || !suite.miss_count || !missFinetuning.engine_ids.length || !missFinetuning.rating_list_id" @click="startMissFinetuning"><template #icon><AppIcon name="gauge" :size="15" /></template>{{ latestMissFinetuning ? 'Finetune again' : 'Finetune misses' }}</BaseButton></div>
            </section>
          </div>
...
.telemetry-actions{align-items:center;display:flex;gap:.5rem}
.miss-stage{grid-column:1/-1}
</style>
