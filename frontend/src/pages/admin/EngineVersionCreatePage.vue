...
async function search(): Promise<void> {
  ...
  selected.value = null
  try {
    const response = await api.get<{ repositories: Repository[] }>('/api/admin/repositories/search', { query: { q: query.value.trim() } })
...
<template>
  <div class="admin-page page-stack">
    ...
    <section class="panel search-panel">
      <div><h2>Find the source repository</h2><p>COPE searches every enabled Git host.</p></div>
    ...
    <form v-if="selected" class="panel source-panel" @submit.prevent="create">
      ...
      <template v-else>
        ...
        <div class="form-grid">
          ...
          <label class="switch-row"><input v-model="active" type="checkbox"><span><strong>Version active</strong><small>Available to new tournaments as soon as its Dockerfile is ready.</small></span></label>
        </div>
