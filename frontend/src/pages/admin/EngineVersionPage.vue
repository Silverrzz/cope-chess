...
const refreshingLocalAvailability = ref(false)
...
async function refreshLocalAvailability(): Promise<void> {
  if (!version.value || !isWorkerLocal.value) return
  refreshingLocalAvailability.value = true
  error.value = ''
  try {
    const response = await api.get<{ version: Engine }>(`/api/admin/engine-versions/${id.value}`)
    version.value.worker_local_count = response.version.worker_local_count ?? 0
    version.value.active = response.version.active
  } catch (cause) {
    error.value = errorText(cause)
    toast.error(cause)
  } finally {
    refreshingLocalAvailability.value = false
  }
}
    ...
    <form v-else-if="version" class="page-stack" @submit.prevent="save">
      <section class="panel detail-card">
        <div class="detail-heading">
          <div><h2>{{ isWorkerLocal ? 'Worker-local binary' : 'Source' }}</h2><p>{{ isWorkerLocal ? 'This private version is never stored, distributed, or benchmarked by the server.' : 'The benchmarker checks out this exact source to build the published worker artifact.' }}</p></div>
          <div class="availability" :class="{ 'availability--active': version.active }">
            <strong>{{ isWorkerLocal && version.active ? 'Tournament eligible' : version.active ? 'Active' : 'Inactive' }}</strong>
            <span class="availability__detail">
              <small>{{ isWorkerLocal ? `${version.worker_local_count ?? 0} connected worker${version.worker_local_count === 1 ? '' : 's'} discovered it` : version.benchmark_current ? (version.engine_active ? 'Current artifact benchmarked' : 'Engine disabled') : 'Current artifact needs a benchmark' }}</small>
              <button v-if="isWorkerLocal" class="availability__refresh" type="button" :disabled="refreshingLocalAvailability" @click="refreshLocalAvailability">{{ refreshingLocalAvailability ? 'Refreshing…' : 'Refresh' }}</button>
            </span>
          </div>
        </div>
        <div class="form-grid">
        ...
        <div v-if="isWorkerLocal" class="local-engine-guidance">
          <strong>Ready for tournament selection</strong>
          <span>Games using this version stay queued until a connected worker reports the matching local key. Connected workers rescan the local engine directory while online.</span>
        </div>
      </section>
