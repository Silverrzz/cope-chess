...
interface Response { rating_list: { id: number; name: string; created_at: string }; ratings: RatingRow[]; tournaments: Commit[] }
...
async function load() { loading.value = true; error.value = ''; try { data.value = await api.get<Response>(`/api/admin/rating-lists/${id.value}`) } catch (cause) { error.value = errorText(cause) } finally { loading.value = false } }
async function uncommit(item: Commit) {
  if (!await confirm({ title: 'Uncommit tournament?', message: `Remove ${item.tournament_name} from this list and recalculate it? The tournament and games are retained.`, confirmLabel: 'Uncommit', tone: 'danger' })) return
...
async function recalculate() { pending.value = 'recalculate'; try { const response = await api.post<{message:string}>('/api/admin/ratings/recalculate', {}); toast.success(response.message); await load() } catch (cause) { error.value = errorText(cause) } finally { pending.value = '' } }
...
<style scoped>.loading{min-height:16rem;padding:2rem}.table-panel{overflow:hidden;padding:0}.heading{align-items:center;border-bottom:1px solid var(--color-border);display:flex;justify-content:space-between;padding:.9rem 1rem}.heading h2{font-size:.9rem;margin:0}.heading span{color:var(--color-text-muted);font-size:.7rem}.table-wrap{overflow:auto}table{border-collapse:collapse;min-width:48rem;width:100%}th,td{border-bottom:1px solid var(--color-border);font-size:.72rem;padding:.72rem .8rem;text-align:left}th{color:var(--color-text-muted);font-size:.62rem;text-transform:uppercase}tbody tr:last-child td{border-bottom:0}</style>
