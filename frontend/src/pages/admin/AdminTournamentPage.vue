    ...
    <template v-else-if="data">
      ...
      <section class="panel control-bar">
        ...
        <div class="control-bar__actions">
          ...
          <button v-for="(_, action) in data.actions" :key="action" class="button" :class="action === 'abort' ? 'button--danger' : 'button--primary'" type="button" :disabled="!!pending" @click="changeStatus(String(action))">{{ pending === action ? (action === 'pause' ? 'Finishing current moves…' : 'Working…') : humanize(String(action)) }}</button>
