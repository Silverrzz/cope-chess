    ...
    <template v-else-if="data">
      ...
      <section v-if="data.failures.length" class="panel failure-panel" aria-labelledby="worker-failures-title">
        <div class="failure-panel__heading">
          <div><h2 id="worker-failures-title">Engine failures</h2><p>The newest download, verification, or runtime failure is shown first.</p></div>
      ...
      <div class="worker-grid">
        ...
        <section class="panel detail-card"><div class="detail-card__heading"><h2>Dynamic capacity</h2></div><dl class="fact-list"><div><dt>CPU</dt><dd>{{ data.worker.hw ? `${data.worker.hw.physical_cores} physical cores` : 'Reported on first connection' }}</dd></div><div><dt>Memory</dt><dd>{{ data.worker.hw ? `${formatNumber(data.worker.hw.ram_mb ?? data.worker.hw.ram_gb * 1024)} MB` : 'Reported on first connection' }}</dd></div><div><dt>Machine ID</dt><dd>{{ data.worker.machine_id ?? 'Reported on first connection' }}</dd></div></dl><p class="capacity-note">The scheduler fills available capacity with independent games and releases each game’s CPU and hash allocation when its engine processes stop.</p></section>
